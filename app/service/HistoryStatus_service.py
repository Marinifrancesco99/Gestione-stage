...
class HistoryStatusService:
    ...
    def get_history_status_by_id(status_id):
        ...
        if not status:
            raise NotFoundException(f"History status with ID {status_id} not found.")
        return status
    ...
    def get_by_internship(internship_id):
        ...
        if not statues:
            raise NotFoundException(f"No history statuses found for internship ID {internship_id}.")
        return statues
